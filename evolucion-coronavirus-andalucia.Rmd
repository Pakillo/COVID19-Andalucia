...
ggplot(datos.andal.long) +
  ...
  theme(legend.position = "none") +
  scale_x_date(limits = c(as.Date(Sys.Date() - 15), NA), 
...
**DATOS DEL DÍA:** `r Sys.Date() - 1`

```{r datos.last}
datos.minist <- read_csv2("datos/ultimos_datos.csv", 
                        col_types = "Dcci-",
                          locale = locale(date_names = "es", 
                                          decimal_mark = ",",
                                          grouping_mark = ".",
...
datos.last <- datos.minist %>%
  group_by(Territorio) %>%
  mutate_at(3:7, function(x) {x - lag(x)}) %>%
...
*(valores negativos significan que se han eliminado casos respecto al día anterior)*
...
ggplot(datos.andal) +
  ...
  scale_x_date(limits = c(as.Date(Sys.Date() - 15), NA), 
               date_breaks = "2 days", date_labels = "%d %B") +
  geom_text(aes(Fecha, y = ConfirmadosPCR + 2, label = ConfirmadosPCR),
            data = subset(datos.andal, Fecha > (Sys.Date() - 15))) 
...
ggplot(datos.andal) +
  ...
  scale_x_date(limits = c(Sys.Date() - 15, NA), 
               date_breaks = "2 days", date_labels = "%d %B") +
  geom_text(aes(Fecha, y = Hospitalizados + 1, label = Hospitalizados),
            data = subset(datos.andal, Fecha > (Sys.Date() - 15))) 
...
ggplot(camas) +
  ...
  labs(title = "Camas ocupadas con COVID-19 en hospitales andaluces",
       caption = "https://tiny.cc/COVID19-Andalucia",
       x = "", y = "Camas ocupadas\n") +
  geom_text(data = camas[nrow(camas), ], aes(Fecha + 0.1, CamasOcup, label = "Totales"), colour = "#377eb8") +
  geom_text(data = camas[nrow(camas), ], aes(Fecha + 0.1, UCI, label = "UCI"), colour = "#e41a1c")
...
ggplot(cfr) +
  ...
  coord_flip() +
  labs(y = "Case Fatality Rate (%)", x = "Edad\n", 
...
ccaa.pcr <- readr::read_csv("https://raw.githubusercontent.com/datadista/datasets/master/COVID%2019/ccaa_covid19_confirmados_pcr_long.csv", guess_max = 10000) 

library(plotly)

options(scipen = 1000)

ggplotly(ggplot(ccaa.pcr) +
  # geom_smooth(aes(x = Fecha, y = PCR.new, group = CCAA), 
  #             colour = "grey80", se = FALSE, span = 0.1) +
  geom_line(aes(x = fecha, y = total, group = CCAA), 
              colour = "grey80") +
...
```{r hosp.ccaa, fig.height=5, out.width='100%'}

ccaa.hosp <- readr::read_csv("https://raw.githubusercontent.com/datadista/datasets/master/COVID%2019/ccaa_covid19_hospitalizados_long.csv", guess_max = 10000)

ggplotly(ggplot(ccaa.hosp) +
  # geom_smooth(aes(x = Fecha, y = PCR.new, group = CCAA), 
  #             colour = "grey80", se = FALSE, span = 0.1) +
  geom_line(aes(x = fecha, y = total, group = CCAA), 
              colour = "grey80") +
  geom_line(aes(x = fecha, y = total), 
            data = subset(ccaa.hosp, CCAA == "Andalucía"), 
            colour = "#377eb8", size = 2) +
  scale_y_log10() +
  labs(x = "", y = "Nº Ingresos\n",
       title = "Nuevos ingresos con COVID-19 por CCAA",
       subtitle = "Escala logarítmica",
       caption = "https://tiny.cc/COVID19-Andalucia"))
...
```{r def.ccaa, fig.height=4, out.width='100%'}

ccaa.def <- readr::read_csv("https://raw.githubusercontent.com/datadista/datasets/master/COVID%2019/ccaa_covid19_fallecidos_long.csv", guess_max = 10000)

ggplotly(ggplot(ccaa.def) +
  geom_line(aes(x = fecha, y = total, group = CCAA), 
              colour = "grey80") +
  geom_line(aes(x = fecha, y = total), 
            data = subset(ccaa.def, CCAA == "Andalucía"), 
            colour = "grey20", size = 2) +
  scale_y_log10() +
  labs(x = "", y = "Defunciones\n",
       title = "Defunciones relacionadas con COVID-19 por CCAA",
       subtitle = "Escala logarítmica",
       caption = "https://tiny.cc/COVID19-Andalucia"))
...
Realizado por [Francisco Rodríguez Sánchez](https://twitter.com/frod_san) a partir de los [datos](https://github.com/Pakillo/COVID19-Andalucia/tree/master/datos) ofrecidos por la Consejería de Salud y Familias de la Junta de Andalucía. Reproducción permitida con fines no comerciales citando la fuente (CC-BY-NC 4.0).
