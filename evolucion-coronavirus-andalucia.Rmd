...
indic.color.riesgo <- Vectorize(function(value, umbrales = indic.umbrales) {

  color <- umbrales$Color[umbrales$Riesgo == value]
  
}, vectorize.args = "value")
# 
# calc_riesgo <- function(value, umbrales = indic.umbrales) {
#   
#   riesgo <- "NORMAL"
#   riesgo <- ifelse(value > )
# }
...
datos.indic %>% 
  select(-Fecha) %>% 
  select(Territorio, Riesgo, everything()) %>% 
  flextable() %>% 
...
Datos referidos al `r unique(datos.indic$Fecha)`
