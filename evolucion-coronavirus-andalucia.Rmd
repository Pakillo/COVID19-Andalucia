...
ggplot(andal) +
  ...
  scale_y_continuous(limits = c(0, max(andal$Fallecimientos.dia, na.rm = TRUE)),
                     breaks = seq(0, max(andal$Fallecimientos.dia, na.rm = TRUE),
                                  by = 2)) +
...
ggplot(datos) +
  ...
  scale_y_continuous(limits = c(0, max(datos$Fallecimientos.dia, na.rm = TRUE)),
                     breaks = seq(0, max(datos$Fallecimientos.dia, na.rm = TRUE),
                                  by = 1)) +
...
Realizado por [Francisco Rodríguez Sánchez](https://twitter.com/frod_san) y Manuel Rodríguez Brea, a partir de los [datos]((https://www.juntadeandalucia.es/organismos/saludyfamilias/areas/salud-vida/paginas/Nuevo_Coronavirus.html)) ofrecidos por la Consejería de Salud y Familias de la Junta de Andalucía.
