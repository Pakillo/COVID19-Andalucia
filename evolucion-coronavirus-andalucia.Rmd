...
Aquí se visualiza la evolución de la enfermedad del coronavirus (COVID-19) en Andalucía, según los [datos oficiales ofrecidos por la Consejería de Salud y Familias](https://www.juntadeandalucia.es/organismos/saludyfamilias/areas/salud-vida/paginas/Nuevo_Coronavirus.html). 
...
Debe tenerse en cuenta que el número de casos detectados depende en gran medida del número de tests realizados. Por tanto, un aumento o disminución en el número de casos detectados cada día puede deberse a un mayor o menor esfuerzo en la realización de tests, no a variaciones en la incidencia de la enfermedad.
...
ggplot(datos) +
  ...
  labs(x = "", y = "Nuevas detecciones\n",
       title = "Nuevos casos detectados diariamente por provincia") 
...
Número neto de personas ingresadas en centros sanitarios de Andalucía cada día, teniendo en cuenta los nuevos ingresos así como las altas hospitalarias.
...
ggplot(datos) +
  ...
  labs(x = "", y = "Nº Personas\n",
       title = "Personas ingresadas por provincia")
...
ggplot(andal) +
  geom_line(aes(x = Fecha, y = Fallecimientos.dia), size = 2) +
  ylim(0, max(andal$Fallecimientos.dia)) +
...
```{r fallecidos_provincias, eval = FALSE}
         
ggplot(datos) +
  facet_wrap(~Provincia, scales = "free_y", ncol = 2) +
  geom_line(aes(x = Fecha, y = Fallecimientos.dia), size = 1.5) +
  ylim(0, max(datos$Fallecimientos.dia)) +
  labs(x = "", y = "Nº Fallecidos\n",
       title = "Fallecimientos diarios por COVID-19 por provincia")
```
