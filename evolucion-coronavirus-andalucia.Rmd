...
ggplot(camas) +
  ...
  annotate("text", x = max(camas.last$Fecha) + 28, y = camas.last$CamasOcup, 
           label = paste0("Total ", camas.last$CamasOcup), colour = "#377eb8") +
  annotate("text", x = max(camas.last$Fecha) + 28, y = camas.last$CamasUCI,
...
ggplot() +
  ...
  labs(title = "\nPacientes ingresados con COVID-19 en Andalucía",
       caption = "https://tiny.cc/COVID19-Andalucia",
       x = "", y = "Camas ocupadas\n") +
  annotate("text", x = max(camas.last$Fecha) + 4, y = camas.last$CamasOcup, 
           label = paste0("Total ", camas.last$CamasOcup), colour = "#377eb8") +
  annotate("text", x = max(camas.last$Fecha) + 4, y = camas.last$CamasUCI,
           label = paste0("UCI ", camas.last$CamasUCI), colour = "#e41a1c") +
  scale_x_date(date_breaks = "1 month", date_labels = "%b", expand = c(0, 5)) 
...
En esta figura interactiva se muestra la incidencia acumulada a 14 días (IA) en cada municipio de Andalucía, y el cambio que dicho valor de IA representa respecto a la IA de catorce días previos. Cambios negativos implican un descenso de la IA en las últimas dos semanas, y cambios positivos implican aumentos de la IA. Pase el cursor sobre los puntos para ver el nombre del municipio.
...
ia.cambio <- muni.todo %>% 
  filter(Fecha == max(Fecha) - 1) %>% 
  # filter(Poblacion > 5000) %>% 
  filter(IA.14d < 1000, IA.cambio < 200) %>% 
  ggplot() +
...
En esta gráfica animada e interactiva se puede examinar con más detalle la evolución de la Incidencia Acumulada en las últimas semanas en **municipios de > 10.000 habitantes**. Pulsando sobre los nombres de provincia en la leyenda de la derecha se pueden resaltar/desactivar los municipios de cada provincia.
...
casos.edad.fecha %>%
  ...
  geom_line(aes(x = Fecha, y = casos_ma, colour = edades),
            size = 1.5) +
  scale_x_date(limits = c(as.Date(NA), max(casos.edad.fecha$Fecha) - 4), 
               date_breaks = "1 month", date_labels = "%b") +
  labs(y = "Casos\n", x = "",
...
Aquí se representan los mismos datos (ingresos diarios por franja de edad) usando medias móviles a 7 días para facilitar la visualización de tendencias:
...
ing.lines <- datos.diarios.iscii.notna %>% 
  ...
  group_by(edades) %>% 
  mutate(hosp_ma = zoo::rollmean(hosp, 7, fill = NA)) %>% 
  ungroup() 
...
ggplot(ing.lines) +
  geom_line(aes(x = fecha, y = hosp_ma, colour = edades),
            size = 1.5) +
  scale_x_date(limits = c(as.Date(NA), max(datos.diarios.iscii.notna$fecha) - 5), 
               date_breaks = "1 month", date_labels = "%b") +
  labs(y = "Ingresos\n", x = "",
...
ggplot() +
  geom_line(aes(x = fecha, y = hosp_ma, colour = edades),
            size = 1.5) +
  scale_x_date(limits = c(as.Date(NA), max(ing.lines$fecha) - 5),
...
ggplot(uci.edad) +
  ...
  geom_text(aes(x = edades.f, y = n + 20, label = n), size = 3) +
  labs(x = "Edad (años)", y = "", 
...
uci.lines <- datos.diarios.iscii.notna %>% 
  ...
  mutate(edades = factor(grupo_edad, levels = unique(grupo_edad))) %>% 
  group_by(edades) %>% 
  mutate(uci_ma = zoo::rollmean(uci, 14, fill = NA)) %>% 
  ungroup() 
...
ggplot(uci.lines) +
  geom_line(aes(x = fecha, y = uci_ma, colour = edades),
            size = 1.5) +
  scale_x_date(limits = c(as.Date("2021-01-01"), max(datos.diarios.iscii.notna$fecha) - 4), 
               date_breaks = "1 month", date_labels = "%b") +
  labs(y = "Ingresos UCI\n", x = "",
       title = "Ingresos diarios en UCI por franja de edad",
       subtitle = "Medias móviles a 14 días",
       caption = "https://tiny.cc/COVID19-Andalucia",
       colour = "Edad") +
  scale_color_brewer(type = "seq", palette = "YlGnBu") +
  theme(legend.title = element_text(hjust = 0.5))
...
#   geom_line(aes(x = fecha, y = uci_ma, colour = edades),
#             size = 1.5) +
#   scale_x_date(limits = c(as.Date(NA), max(uci.lines$fecha) - 4), 
#                date_breaks = "1 week", date_labels = "%d %b") +
#   labs(y = "Ingresos UCI\n", x = "",
#        title = "Ingresos diarios en UCI por franja de edad",
#        subtitle = "Medias móviles a 14 días",
...
ggplot(def.edad) +
  ...
  geom_text(aes(x = edades.f, y = n + 50, label = n), size = 3) +
  labs(x = "Edad (años)", y = "", 
...
datos.diarios.iscii.notna %>% 
  ...
  mutate(edades = factor(grupo_edad, levels = unique(grupo_edad))) %>% 
  group_by(edades) %>% 
  mutate(def_ma = zoo::rollmean(def, 7, fill = NA)) %>% 
  ungroup() %>% 
  ggplot() +
