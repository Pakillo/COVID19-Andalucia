...
ggplot() +
  ...
  theme(panel.grid.minor.y = element_line(size = rel(0.5))) +
  scale_x_date(date_breaks = "2 days", date_labels = "%d %b") +
  geom_text(aes(Fecha, y = ConfirmadosPCR + 40, label = ConfirmadosPCR),
...
ggplot(camas) +
  ...
  geom_text(data = camas.last, 
            aes(Fecha - 2, CamasOcup + 60, label = paste0( "Total ", CamasOcup)),
            colour = "#377eb8") +
  geom_text(data = camas.last, 
            aes(Fecha - 2, UCI + 60, label = paste0("UCI ", UCI)), 
            colour = "#e41a1c") 
