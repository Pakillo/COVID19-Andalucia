...
ggplot(camas) +
  ...
  annotate("text", x = max(camas.last$Fecha) + 25, y = camas.last$CamasUCI,
...
```{r tabla.datos.dia, out.width='100%', crop = TRUE}
...
mobidata %>% 
  ...
  group_by(place) %>% 
  mutate(change_ma = zoo::rollmean(change, k = 14, fill = NA)) %>% 
  ungroup() %>% 
  ...
  labs(x = "", y = "Diferencia en la movilidad (%)\n",
       ...
       subtitle = "Medias móviles a 14 días de la diferencia en uso del espacio\nrespecto a la mediana diaria del período 3 Ene - 6 Feb 2020",
       caption = "https://tiny.cc/COVID19-Andalucia") +
  theme(legend.position = "none") +
...
ggplot() +
  geom_line(aes(x = Fecha, y = casos_ma, colour = edades),
            size = 1.5) +
  scale_x_date(limits = c(as.Date("2020-07-01"), max(casos.edad.fecha$Fecha) - 4), 
               date_breaks = "1 month", date_labels = "%b") +
  labs(y = "Casos\n", x = "",
...
datos.diarios.iscii.notna %>% 
  ...
  geom_line(aes(x = fecha, y = hosp_ma, colour = edades),
            size = 1.5) +
  scale_x_date(limits = c(as.Date("2020-07-01"), max(datos.diarios.iscii.notna$fecha) - 4), 
               date_breaks = "1 month", date_labels = "%b") +
  labs(y = "Ingresos\n", x = "",
...
datos.diarios.iscii.notna %>% 
  ...
  mutate(edades = factor(grupo_edad, levels = unique(grupo_edad))) %>% 
  group_by(edades) %>% 
  mutate(uci_ma = zoo::rollmean(uci, 7, fill = NA)) %>% 
  ungroup() %>% 
  ggplot() +
