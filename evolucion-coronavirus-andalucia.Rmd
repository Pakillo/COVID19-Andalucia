...
# Nº personas >=65 años: 1470813
# Datos de 2020 (https://www.juntadeandalucia.es/institutodeestadisticaycartografia/badea/operaciones/consulta/anual/21974?CodOper=b3_151&codConsulta=21974)
# 2021: 1495393

casos.65 <- casos.edad.fecha %>% 
  filter(edades %in% c("65", "66", "67", "68", "69", "70 - 79", "80 y más")) %>% 
  group_by(Fecha) %>% 
  # summarise(Casos = sum(Casos)) %>% 
  # mutate(Casos.acum = cumsum(Casos)) %>% 
  summarise(Casos.14d.65a = sum(Casos.14d),
         Casos.7d.65a = sum(Casos.7d)) %>% 
  mutate(Tasa.14d.65a = Casos.14d.65a * 100000 / 1495393,
         Tasa.7d.65a = Casos.7d.65a * 100000 / 1495393) %>% 
...
casos.edad.provs.new <- read_csv2("datos/casos_edad_provs_new.csv",
                              skip_empty_rows = TRUE) %>%
...
## IA mayores 65 años
ia.provs.65yr <- casos.edad.provs.new %>% 
  filter(Edad %in% c("De 65 años", "De 66 años", "De 67 años", "De 68 años", 
                     "De 69 años", "De 70 a 79 años", "De 80 y más años")) %>% 
  group_by(Territorio) %>% 
...
test.minist <- read_csv2("datos/datos_test_ministerio.csv",
  ...
  mutate(N.PCR.TA = N_ANT + N_PCR,
         NPosit = N_ANT_POSITIVOS + N_PCR_POSITIVOS,
         Positividad = round(100*NPosit/N.PCR.TA, digits = 1)) %>% 
  rename(Territorio = PROVINCIA) %>% 
  select(Fecha, Territorio, N.PCR.TA, NPosit, Positividad)

test.minist.andal <- test.minist %>% 
  group_by(Fecha) %>% 
  summarise(N.PCR.TA = sum(N.PCR.TA),
            NPosit = sum(NPosit),
            Positividad = round(100*NPosit/N.PCR.TA, digits = 1)) %>% 
  mutate(Territorio = "Andalucía") %>% 
  select(Fecha, Territorio, N.PCR.TA, NPosit, Positividad)
...
test.minist <- bind_rows(test.minist, test.minist.andal) %>% 
  arrange(Fecha, Territorio)

test.minist.last.day <- test.minist %>% 
...
# see nhab_edades_2021.xls
nhab.edad <- data.frame(
  grupo_edad = sort(unique(datos.diarios.iscii.notna$grupo_edad)),
  nhab = c(789238L, 956010L, 923357L, 1109748L, 1386044L, 1282786L, 954242L, 649045L, 421937L))
...
# Add datos camas ocupadas to datos.acum

camas2join <- datos.camas.minist %>%
  select(Fecha, Territorio,
         CamasOcup = covid_Convencional, 
         CamasUCI = covid_UCI,
         `%CamasOcup.Minist` = `%OcupHosp`,
         `%UCI.Minist` = `%OcupUCI`) 

datos.acum <- datos.acum %>% 
  left_join(camas2join, by = c("Fecha", "Territorio")) %>% 
  arrange(Fecha)
...
datos.indic <- datos.indic %>% 
  ...
  mutate(IA14 = ia.provs$IA.14d,
         ...
         Positividad = test.minist.last.day$Positividad,
...
Los distintos colores representan [niveles crecientes de riesgo](https://twitter.com/DadosdeLaplace/status/1319326140671066113/photo/1), desde el **verde ("circulación controlada") al morado ("riesgo muy alto")**, pasando por amarillo (riesgo bajo), naranja (riesgo medio), y rojo (riesgo alto). El 23 de Noviembre de 2021 se fijaron [nuevos umbrales](https://www.rtve.es/noticias/20211202/semaforo-coronavirus-nivel-riesgo-covid19-espana-comunidades/2059301.shtml) para algunos indicadores.
...
Datos relativos al `r max(datos.acum$Fecha)`. 
...
ggplot(datos.plot) +
  ...
  annotate("text", x = 15 + max(datos.plot$Fecha),
  ...
  geom_ribbon(aes(x = Fecha, ymin = 0, ymax = 25), fill = "#4daf4a", alpha = 0.3,
              data = filter(datos.plot, Fecha < as.Date("2021-11-23"))) +
  ...
  geom_ribbon(aes(x = Fecha, ymin = 150, ymax = 250), fill = "#e41a1c", alpha = 0.3,
              data = filter(datos.plot, Fecha < as.Date("2021-11-23"))) +
  geom_ribbon(aes(x = Fecha, ymin = 250, ymax = max(datos.plot$IA14)), 
              fill = "#984ea3", alpha = 0.3,
              data = filter(datos.plot, Fecha < as.Date("2021-11-23"))) +
  geom_vline(xintercept = as.Date("2021-11-23"), colour = "grey30") +
  geom_ribbon(aes(x = Fecha, ymin = 0, ymax = 50), fill = "#4daf4a", alpha = 0.3,
              data = filter(datos.plot, Fecha > as.Date("2021-11-23"))) +
  geom_ribbon(aes(x = Fecha, ymin = 50, ymax = 100), fill = "#ffff33", alpha = 0.3,
              data = filter(datos.plot, Fecha > as.Date("2021-11-23"))) +
  geom_ribbon(aes(x = Fecha, ymin = 100, ymax = 300), fill = "#ff7f00", alpha = 0.3,
              data = filter(datos.plot, Fecha > as.Date("2021-11-23"))) +
  geom_ribbon(aes(x = Fecha, ymin = 300, ymax = 500), fill = "#e41a1c", alpha = 0.3,
              data = filter(datos.plot, Fecha > as.Date("2021-11-23"))) +
  geom_ribbon(aes(x = Fecha, ymin = 500, ymax = max(datos.plot$IA14)), 
              fill = "#984ea3", alpha = 0.3,
              data = filter(datos.plot, Fecha > as.Date("2021-11-23"))) +
  labs(x = "", y = "",
...
ggplot(datos.plot) +
  ...
  geom_ribbon(aes(x = Fecha, ymin = 75, ymax = 125), fill = "#e41a1c", alpha = 0.3,
              data = filter(datos.plot, Fecha < as.Date("2021-11-23"))) +
  geom_ribbon(aes(x = Fecha, ymin = 125, ymax = max(datos.plot$IA7)), 
              fill = "#984ea3", alpha = 0.3,
              data = filter(datos.plot, Fecha < as.Date("2021-11-23"))) +
  geom_vline(xintercept = as.Date("2021-11-23"), colour = "grey30") +
  geom_ribbon(aes(x = Fecha, ymin = 0, ymax = 25), fill = "#4daf4a", alpha = 0.3,
  ...
  geom_ribbon(aes(x = Fecha, ymin = 150, ymax = 250), fill = "#e41a1c", alpha = 0.3,
              data = filter(datos.plot, Fecha > as.Date("2021-11-23"))) +
  geom_ribbon(aes(x = Fecha, ymin = 250, ymax = max(datos.plot$IA7)), 
              fill = "#984ea3", alpha = 0.3,
              data = filter(datos.plot, Fecha > as.Date("2021-11-23"))) +
  labs(x = "", y = "",
...
```{r IA14.65a, fig.height=4, eval=T}
...
ggplot(datos.plot) +
  ...
  geom_ribbon(aes(x = Fecha, ymin = 100, ymax = 150), fill = "#e41a1c", alpha = 0.3,
              data = filter(datos.plot, Fecha < as.Date("2021-11-23"))) +
  geom_ribbon(aes(x = Fecha, ymin = 150, ymax = max(datos.plot$Tasa.14d.65a)), 
              fill = "#984ea3", alpha = 0.3,
              data = filter(datos.plot, Fecha < as.Date("2021-11-23"))) +
  geom_vline(xintercept = as.Date("2021-11-23"), colour = "grey30") +
  geom_ribbon(aes(x = Fecha, ymin = 0, ymax = 25), fill = "#4daf4a", alpha = 0.3,
  ...
  geom_ribbon(aes(x = Fecha, ymin = 150, ymax = 250), fill = "#e41a1c", alpha = 0.3,
              data = filter(datos.plot, Fecha > as.Date("2021-11-23"))) +
  geom_ribbon(aes(x = Fecha, ymin = 250, ymax = max(datos.plot$Tasa.14d.65a)), 
              fill = "#984ea3", alpha = 0.3,
              data = filter(datos.plot, Fecha > as.Date("2021-11-23"))) +
  labs(x = "", y = "",
...
ggplot(datos.plot) +
  ...
  geom_ribbon(aes(x = Fecha, ymin = 50, ymax = 75), fill = "#e41a1c", alpha = 0.3,
              data = filter(datos.plot, Fecha < as.Date("2021-11-23"))) +
  geom_ribbon(aes(x = Fecha, ymin = 75, ymax = max(datos.plot$Tasa.7d.65a)), 
              fill = "#984ea3", alpha = 0.3,
              data = filter(datos.plot, Fecha < as.Date("2021-11-23"))) +
  geom_vline(xintercept = as.Date("2021-11-23"), colour = "grey30") +
  geom_ribbon(aes(x = Fecha, ymin = 0, ymax = 10), fill = "#4daf4a", alpha = 0.3,
  ...
  geom_ribbon(aes(x = Fecha, ymin = 75, ymax = 125), fill = "#e41a1c", alpha = 0.3,
              data = filter(datos.plot, Fecha > as.Date("2021-11-23"))) +
  geom_ribbon(aes(x = Fecha, ymin = 125, ymax = max(datos.plot$Tasa.7d.65a)), 
              fill = "#984ea3", alpha = 0.3,
              data = filter(datos.plot, Fecha > as.Date("2021-11-23"))) +
  labs(x = "", y = "",
...
datos.plot <- datos.acum.andal %>% 
  filter(!is.na(Positividad)) %>% 
  filter(Fecha >= as.Date("2021-01-01")) %>% 
  mutate(Positividad.ma = zoo::rollmean(Positividad, 7, fill = NA, align = "right"))

ggplot(datos.plot) +
  geom_line(aes(Fecha, Positividad.ma), size = 2) +
  annotate("text", x = 15 + max(datos.plot$Fecha), 
           y = datos.plot$Positividad.ma[nrow(datos.plot)],
           label = paste(round(datos.plot$Positividad.ma[nrow(datos.plot)]), "%", sep = " ")) +
  ...
  geom_ribbon(aes(x = Fecha, ymin = 15, ymax = 40), 
              fill = "#984ea3", alpha = 0.3) +
  labs(x = "", y = "",
       title = "Positividad de las PCR + Test Antígeno",
       subtitle = "Media móvil a 7 días",
       caption = "https://tiny.cc/COVID19-Andalucia") +
...
datos.plot <- datos.acum.andal %>% 
  filter(!is.na(N.PCR.TA))  %>% 
  filter(Fecha >= as.Date("2021-01-01")) %>% 
  mutate(N.PCR.TA.ma = zoo::rollmean(N.PCR.TA, 7, fill = NA, align = "right"))

ggplot(datos.plot) +
  geom_line(aes(Fecha, N.PCR.TA.ma), size = 2) +
  annotate("text", x = 20 + max(datos.plot$Fecha), 
           y = datos.plot$N.PCR.TA.ma[nrow(datos.plot)],
           label = round(datos.plot$N.PCR.TA.ma[nrow(datos.plot)])) +
  labs(x = "", y = "",
       title = "Número de pruebas realizadas (PCR + Test Antígeno)",
       subtitle = "Media móvil a 7 días",
       caption = "https://tiny.cc/COVID19-Andalucia") +
  scale_x_date(date_breaks = "1 month", date_labels = "%b") +
  theme(axis.text.x = element_text(size = 10))
...
```{r Trazabilidad, fig.height=4, eval=FALSE}
# El indicador de Trazabilidad dejó de usarse a partir del 23 Nov 2021 
...
# Datos camas Minist
camas <- datos.camas.minist %>% 
  filter(Territorio == "Andalucía") %>% 
  mutate(CamasOcup = covid_Convencional + covid_UCI,
         CamasUCI = covid_UCI)
...
camas %>% 
  filter(Fecha >= as.Date("2022-02-01")) %>% 
ggplot() +
  geom_line(aes(Fecha, CamasOcup), colour = "#377eb8", size = 2) +
  geom_line(aes(Fecha, CamasUCI), colour = "#e41a1c", size = 2) +
  ylim(0, NA) +
  labs(title = "\nPacientes ingresados con COVID-19 en Andalucía",
       caption = "https://tiny.cc/COVID19-Andalucia",
       x = "", y = "Camas ocupadas\n") +
  annotate("text", x = max(camas.last$Fecha) + 1, y = camas.last$CamasOcup, 
           label = paste0("Total ", camas.last$CamasOcup), colour = "#377eb8",
           hjust = 0) +
...
vacu.edades <- read_csv2("datos/vacu_edades.csv") %>% 
  ...
  filter(!is.na(edades)) %>% 
  mutate(edades = factor(edades, 
                         levels = c("5 a 11", "12 a 19", "20 a 29", "30 a 39",
...
## Maximum values
# datos.andal %>% 
#   summarise(across(ConfirmadosPCR:Defunciones, max, na.rm = TRUE))
# Casos: 8787 (2021-01-22)
# Hosp: 536 (2021-01-22)
# UCI: 59 (2021-01-19)

datos.andal.plot <- datos.andal %>% 
  mutate(Casos = zoo::rollmean(ConfirmadosPCR, 7, fill = NA, align = "right"),
         Hosp = zoo::rollmean(Hospitalizados, 7, fill = NA, align = "right"),
         UCIma = zoo::rollmean(UCI, 7, fill = NA, align = "right"),
         Def = zoo::rollmean(Defunciones, 7, fill = NA, align = "right")) %>% 
  filter(Fecha >= as.Date("2021-01-01"), Fecha < max(Fecha) - 4)

ggplot(datos.andal.plot) +
  geom_line(aes(Fecha, Casos), colour = "#ff7f00", lwd = 2) +
  geom_line(aes(Fecha, Hosp), colour = "#377eb8", lwd = 2) +
  geom_line(aes(Fecha, UCIma), colour = "#e41a1c", lwd = 2) +
  # geom_line(aes(Fecha, Def), colour = "grey40", lwd = 2, alpha = 0.6) +
  # coord_cartesian(xlim = c(as.Date("2021-01-01"), max(datos.andal.plot$Fecha) - 3)) +
  theme(axis.title = element_blank()) +
  scale_y_log10(breaks = c(1, 3, 5, 10, 30, 50, 100, 300, 500, 1000, 3000, 5000, 10000)) +
  annotate("text", x = as.Date("2021-10-10"), y = 1500, 
           label = "Casos", colour = "#ff7f00", size = 5) +
  annotate("text", x = as.Date("2021-10-15"), y = 60, 
           label = "Ingresos", colour = "#377eb8", size = 5) +
  annotate("text", x = as.Date("2021-10-15"), y = 4, 
           label = "UCI", colour = "#e41a1c", size = 5) +
  labs(caption = "https://tiny.cc/COVID19-Andalucia",
       title = "Evolución de casos confirmados, ingresos totales y UCI",
       subtitle = "Media móvil a 7 días")
...
ggplot() +
  geom_line(aes(Fecha, Casos), colour = "#ff7f00", lwd = 2) +
  geom_line(aes(Fecha, Hosp), colour = "#377eb8", lwd = 2) +
  geom_line(aes(Fecha, Def), colour = "grey40", lwd = 2) +
  theme(axis.title = element_blank()) +
  scale_y_log10(breaks = c(1, 3, 5, 10, 30, 50, 100, 300, 500, 1000, 3000, 5000, 10000)) +
  annotate("text", x = as.Date("2021-10-10"), y = 1500, 
           label = "Casos", colour = "#ff7f00", size = 5) +
  annotate("text", x = as.Date("2021-10-15"), y = 60, 
           label = "Ingresos", colour = "#377eb8", size = 5) +
  annotate("text", x = as.Date("2021-10-20"), y = 6, 
           label = "Defunciones", colour = "grey40", size = 5) +
  labs(caption = "https://tiny.cc/COVID19-Andalucia",
       title = "Evolución de casos confirmados, ingresos y defunciones",
       subtitle = "Media móvil a 7 días")
```
...
Casos confirmados por PCR o test de antígeno en las últimas 3 semanas:
...
datos.andal %>%
  filter(Fecha > max(datos.andal$Fecha) - 21) %>%
ggplot() +
  geom_col(aes(x = Fecha, y = ConfirmadosPCR), 
            fill = "#ff7f00") +
  ...
  labs(x = "", y = "Nuevas detecciones\n",
       title = "Casos diarios de COVID-19 en Andalucía",
       subtitle = "Últimas 3 semanas",
       caption = "https://tiny.cc/COVID19-Andalucia") +
  theme(panel.grid.minor.y = element_line(size = rel(0.5))) +
  scale_x_date(date_breaks = "3 days", date_labels = "%d %b") +
  geom_text(aes(Fecha, y = ConfirmadosPCR + 100, label = ConfirmadosPCR),
            data = subset(datos.andal, Fecha > max(datos.andal$Fecha) - 21), size = 2.5) 
...
datos.provs %>%
  filter(Fecha > as.Date(Sys.Date() - 21)) %>%
ggplot() +
...
if (! file.exists("datos/2021_ES_Region_Mobility_Report.csv")) {
  unzip("datos/Region_Mobility_Report_CSVs.zip", 
        files = c("2021_ES_Region_Mobility_Report.csv", 
                  "2022_ES_Region_Mobility_Report.csv"),
        exdir = "datos")
}

mobidata <- bind_rows(
  read_csv("datos/2021_ES_Region_Mobility_Report.csv"),
  read_csv("datos/2022_ES_Region_Mobility_Report.csv")) %>% 
...
mobidt <- mobidata %>% 
  ...
  group_by(place) %>% 
  mutate(change_ma = zoo::rollmean(change, k = 30, fill = NA, align = "right")) %>% 
  ungroup() %>% 
  filter(date > as.Date("2021-01-06"))

ggplot(mobidt) +
  aes(x = date, y = change_ma) +
  geom_textline(aes(colour = place, label = place), linewidth = 1.5, 
                straight = FALSE, text_smoothing = 40,
                hjust = "auto") + 
...
muni.dia <- muni.dia %>%
    ...
    dplyr::filter(Medida == "Población media anual" | Medida == "Confirmados PDIA" | 
    ...
    select(-rownumber) %>%
    tidyr::pivot_wider(names_from = "Medida", values_from = "Valor") %>%
    rename(`Población` = `Población media anual`) %>%
...
ia.cambio <- muni.todo %>% 
  ...
  filter(IA.14d < 3000, IA.cambio < 1000) %>%
  ggplot() +
...
fig <- muni.todo %>%
  ...
  filter(Poblacion > 5000) %>% 
  # filter(IA.14d < 1000, IA.cambio < 500) %>%
  filter(IA.14d < 3000, IA.cambio < 500) %>%
...
```{r }
casos.edad.provs.new %>% 
  filter(Territorio == "Andalucía", 
         Edad != "TOTAL", 
         !Edad %in% c("De 65 años", "De 66 años", "De 67 años", "De 68 años", 
                     "De 69 años")) %>% 
...
```{r casos.edad.lines, out.width='100%', eval=FALSE}
...
```{r casos.edad.lines.iscii, eval=TRUE}
...
casos.edad.fecha.iscii %>% 
  filter(fecha >= as.Date("2022-02-01")) %>%
  ggplot() +
  # geom_textline(aes(x = fecha, y = casos_ma, colour = grupo_edad, label = grupo_edad),
  #           linewidth = 1.5, hjust = 0.05, straight = TRUE, #text_smoothing = 60, 
  #           show.legend = FALSE) +
  geom_line(aes(x = fecha, y = casos_ma, colour = grupo_edad), 
            size = 1.5, show.legend = FALSE) +
  scale_x_date(limits = c(as.Date(NA), max(casos.edad.fecha.iscii$fecha) - 3),
               expand = c(0, 7),
               date_breaks = "1 week", date_labels = "%d %b") +
  ggrepel::geom_text_repel(data = filter(casos.edad.fecha.iscii,
                                         fecha == max(casos.edad.fecha.iscii$fecha) - 3),
                  aes(x = fecha, y = casos_ma,
                      label = grupo_edad, colour = grupo_edad),
                  min.segment.length = 0,
                  nudge_x = 5, 
                  show.legend = FALSE) +
  labs(y = "Casos\n", x = "",
...
casos.edad.fecha.iscii %>% 
  filter(fecha >= as.Date("2022-02-01")) %>% 
ggplot() +
  # geom_textline(aes(x = fecha, y = IA.7d, colour = grupo_edad, label = grupo_edad),
  #           linewidth = 1.5, #hjust = 0.30, 
  #           straight = TRUE, show.legend = FALSE) +
  geom_line(aes(x = fecha, y = IA.7d, colour = grupo_edad),
            size = 1.5, show.legend = FALSE) +
  ggrepel::geom_text_repel(data = filter(casos.edad.fecha.iscii,
                                         fecha == max(casos.edad.fecha.iscii$fecha) - 3),
                  aes(x = fecha, y = IA.7d,
  ...
  scale_x_date(limits = c(as.Date(NA), max(casos.edad.fecha.iscii$fecha) - 3), 
               ...
               expand = c(0, 7)) +
  labs(y = "Incidencia Acumulada\n", x = "",
       title = "Incidencia Acumulada 7 días por franja de edad",
       subtitle = "Casos confirmados en los últimos 7 días por cada 100.000 habitantes",
       caption = "https://tiny.cc/COVID19-Andalucia",
       colour = "Edad") +
  scale_colour_viridis_d(direction = -1) 
...
ggplot(datos.andal) +
  ...
  labs(x = "", y = "Nº Ingresos\n",
       ...
       subtitle = "Últimas 3 semanas",
       caption = "https://tiny.cc/COVID19-Andalucia") +
  scale_x_date(limits = c(Sys.Date() - 22, NA), 
               date_breaks = "2 days", date_labels = "%d %b") +
  theme(axis.text.x = element_text(size = 8)) +
  geom_text(aes(Fecha, y = Hospitalizados + 10, label = Hospitalizados),
            data = subset(datos.andal, Fecha > (Sys.Date() - 22))) 
...
ggplot(hosp.edad) +
  ...
  geom_text(aes(x = edades.f, y = n + 200, label = n), size = 3) +
  labs(x = "Edad (años)", y = "", 
...
ing.lines <- datos.diarios.iscii.notna %>% 
  ...
  group_by(edades) %>% 
  mutate(hosp_ma_14d = zoo::rollmean(hosp, 14, fill = NA, align = "right"),
         hosp_ma_7d = zoo::rollmean(hosp, 7, fill = NA, align = "right")) %>% 
  ungroup() 


ggplot(ing.lines) +
  geom_line(aes(x = fecha, y = hosp_ma_14d, colour = edades),
            size = 1.5) +
...
ggplot(data = ing.lines.zoom,
       aes(x = fecha, y = hosp_ma_14d)) +
  geom_textline(aes(colour = edades, label = edades),
            linewidth = 1.5, hjust = "auto", alpha = 0.8,
            straight = TRUE,
            # text_smoothing = 30,
            show.legend = FALSE, ) +
...
ggplot(def.edad) +
  ...
  geom_text(aes(x = edades.f, y = n + 80, label = n), size = 3) +
  labs(x = "Edad (años)", y = "", 
