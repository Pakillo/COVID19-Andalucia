...
ggplot() +
  ...
  theme(panel.grid.minor.y = element_line(size = rel(0.5))) +
  scale_x_date(date_breaks = "2 days", date_labels = "%d %B") +
  geom_text(aes(Fecha, y = ConfirmadosPCR + 20, label = ConfirmadosPCR),
...
ggplot(datos.andal) +
  ...
  geom_text(aes(Fecha, y = Hospitalizados + 2, label = Hospitalizados),
            data = subset(datos.andal, Fecha > (Sys.Date() - 16))) 
...
ggplot(camas) +
  ...
  geom_text(data = camas.last, 
            aes(Fecha - 2, CamasOcup + 20, label = paste0( "Total ", CamasOcup)),
            colour = "#377eb8") +
  geom_text(data = camas.last, 
            aes(Fecha - 2, UCI + 20, label = paste0("UCI ", UCI)), 
            colour = "#e41a1c") 
