...
ggplot() +
  ...
  theme(panel.grid.minor.y = element_line(size = rel(0.5))) +
  scale_x_date(date_breaks = "2 days", date_labels = "%d %b") +
  geom_text(aes(Fecha, y = ConfirmadosPCR + 180, label = ConfirmadosPCR),
...
ggplot() +
  geom_line(aes(x = Fecha, y = casos_ma, colour = edades),
            size = 1.5) +
  scale_x_date(limits = c(as.Date("2020-07-01"), max(casos.edad.fecha$Fecha) - 4), expand = c(0, 20),
               date_breaks = "1 month", date_labels = "%b") +
  labs(y = "Casos\n", x = "",
