...
mobidata <- read_csv("./2021_ES_Region_Mobility_Report.csv") %>% 
  ...
  pivot_longer(cols = starts_with("tiendas"):starts_with("residencial"), 
...
mobidata %>% 
  ...
  group_by(place) %>% 
  mutate(change_ma = zoo::rollmean(change, k = 7, fill = NA)) %>% 
  ungroup() %>% 
  ...
  labs(x = "", y = "Diferencia en la movilidad (%)\n",
       ...
       subtitle = "Medias móviles a 7 días de la diferencia en uso del espacio\nrespecto a la mediana diaria del período 3 Ene - 6 Feb 2020",
       caption = "https://tiny.cc/COVID19-Andalucia") +
  theme(legend.position = "none") +
