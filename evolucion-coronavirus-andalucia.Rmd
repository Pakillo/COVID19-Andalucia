...
Este mapa muestra los casos confirmados mediante PCR en los últimos 14 días en cada municipio. 
...
```{r mapamunis_PCR_14d, eval=FALSE}
...
muni.14d <- left_join(select(muni.gpkg, PROVINCIA, MUNICIPIO),
                      select(muni.data, PROVINCIA, MUNICIPIO, ConfirmadosPCR14d),
                      by = c("PROVINCIA", "MUNICIPIO"))


ggplot(muni.14d) +
  geom_sf(aes(fill = ConfirmadosPCR14d))
