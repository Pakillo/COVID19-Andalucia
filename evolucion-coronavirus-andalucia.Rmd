...
casos.65 <- casos.edad.fecha %>% 
  ...
  mutate(Casos.14d = Casos.acum - lag(Casos.acum, n = 13),
         Casos.7d = Casos.acum - lag(Casos.acum, n = 6)) %>% 
...
Las siguientes gráficas muestran la evolución de cada uno de los [indicadores](https://www.scribd.com/document/481185389/Actuaciones-de-respuesta-coordinada-para-el-control-de-la-transmision-de-COVID-19) aprobados por el Consejo Interterritorial de Salud.

Los distintos colores representan niveles crecientes de riesgo, desde el verde ("nueva normalidad") al morado ("riesgo extremo").
...
ggplot(datos.plot) +
  ...
  annotate("text", x = max(datos.plot$Fecha), y = 20 + datos.plot$Tasa.14d[nrow(datos.plot)],
...
ggplot(datos.plot) +
  ...
  annotate("text", x = max(datos.plot$Fecha), y = 10 + datos.plot$Tasa.7d[nrow(datos.plot)],
...
ggplot(datos.plot) +
  ...
  annotate("text", x = 0.4 + max(datos.plot$Fecha), 
  ...
  geom_ribbon(aes(x = Fecha, ymin = 15, ymax = max(Positividad.7d)), 
              fill = "#984ea3", alpha = 0.3) +
  labs(x = "", y = "",
...
ggplot(datos.plot) +
  ...
  annotate("text", x = 0.5 + max(datos.plot$Fecha), 
  ...
  geom_ribbon(aes(x = Fecha, ymin = 10, ymax = 15), fill = "#e41a1c", alpha = 0.3) +
  geom_ribbon(aes(x = Fecha, ymin = 15, ymax = pmax(15, `%CamasOcup.Minist`)), 
              fill = "#984ea3", alpha = 0.3) +
  labs(x = "", y = "",
...
ggplot(datos.plot) +
  ...
  geom_ribbon(aes(x = Fecha, ymin = 25, ymax = pmax(25, `%UCI.Minist`)), 
              fill = "#984ea3", alpha = 0.3) +
  labs(x = "", y = "",
       title = "Ocupación UCI",
...
Aquí se representan los mismos datos (casos confirmados por categoría de edad) usando curvas suavizadas mediante *loess* (local polynomial regression) para reducir la variación diaria y facilitar la visualización de tendencias:
...
ggplot(casos.edad.fecha) +
  ...
  scale_x_date(limits = c(as.Date("2020-07-01"), Sys.Date() - 4), expand = c(0, 20),
               date_breaks = "1 month", date_labels = "%b") +
  labs(y = "Casos\n", x = "",
       title = "Casos confirmados por categoría de edad",
       caption = "https://tiny.cc/COVID19-Andalucia",
       colour = "Edad") +
...
El pico máximo de camas ocupadas se alcanzó el 30 de Marzo con 2708 pacientes ingresados (438 en UCI).
...
ggplot() +
  ...
  geom_text(data = camas.last, 
            aes(Fecha - 2, CamasOcup + 250, label = paste0( "Total ", CamasOcup)),
            colour = "#377eb8") +
  geom_text(data = camas.last, 
            aes(Fecha - 2, CamasUCI + 200, label = paste0("UCI ", CamasUCI)), 
            colour = "#e41a1c") +
  scale_x_date(date_breaks = "1 month", date_labels = "%b")
